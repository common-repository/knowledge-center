...
if (!function_exists('emd_show_shortcodes_page')) {
	function emd_show_shortcodes_page($app){
		...
		if(function_exists('emd_std_media_js') || function_exists('emd_analytics_media_js')){	
			...
			echo '<a href="' . esc_url($create_url) . '" class="thickbox button button-primary">' . esc_html('Create New', 'emd-plugins') . '</a>';
			echo '</h2>';
			...
			echo '</h2>';
			echo '<p>' . __('The following shortcodes are provided by default. To use the shortcode, click copy button and paste it in a page.','emd-plugins');
			echo ' ' . sprintf(__('To learn more on how to create new shortcodes with filters go to the %s documentation.%s','emd-plugins'),'<a href="https://docs.emdplugins.com/docs/' . esc_attr(str_replace('_','-',$app)) . '" target="_blank">','</a>') . '</p>';
